...
extern int mv_linux_mux_initialize(void);
#endif
...
//    MV_U32 boardId= mvBoardIdGet();
    ...
    hal_init_struct.mru = MV_ETH_MRU_ILLEGAL;
 ...
 *                   structure                             *
 ***********************************************************/
extern void    mvPpFtdllWaXcat2(void);
struct net_device* __init mv_netdev_init(mv_eth_priv *priv, int mtu, u8* mac)
{
    ...
    if (mvPpChipIsXCat2() == MV_TRUE && mvChipXCat2IsBGA() == MV_TRUE)
    {
        ...
        {
            mvPpWriteReg(0, 0x000000B0, 0x00000481);
        }
    }
...
static int __init mv_eth_init_module( void ) 
{
...
#ifdef MV_PRESTERA_SWITCH
        /*Init Prestera HAL functionality*/
        if (mvPpHalInitHw() != MV_OK)
        {
            mvOsPrintf("%s: mvPpHalInitHw failed.\n", __func__);
        }
        /*start prestera switch access and network services*/
		mv_linux_mux_initialize();
#endif

    if(( featureSetP->numOfGbe == 1 ) && mvBoardIsOOBAvalilable())
	{
    	printk( "Marvell Gbe Ethernet Driver not supported\n");
		return 0;
	}
    ...
    for (port = 0; port < mv_eth_ports_num; port++)
    {
        ...
        if (port == 1) 
        {
            printk("\nWarning: Giga %d is Forced Skipped Off\n", port);
            continue;
        }
