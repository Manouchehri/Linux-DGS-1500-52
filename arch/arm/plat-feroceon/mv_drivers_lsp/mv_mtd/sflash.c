...
static struct mtd_info *sflash_probe(struct map_info *map)
{
    ...
    mvSflashInit.numOfSpiCS = featureSetP->numOfSpiCS;
    /* Ref # RES-SPI-20 (Chip Select (SPI_CSn) Deselect Timing) */
    /* SPI fix for xCat-A1 */
    if (mvBoardChipIsXCat() == MV_TRUE && mvBoardChipRevGet() == 1)
    {
        DB(mvOsPrintf("%s: SPI fix for xCat-A1 is activated.\n", __func__));
        mvSflashInit.isCsAssertDelayNeeded = MV_TRUE;
    }
    else
    {
        DB(mvOsPrintf("%s: SPI fix for xCat-A1 is NOT activated.\n", __func__));
        mvSflashInit.isCsAssertDelayNeeded = MV_FALSE;
    }
    mvSflashInit.csAssertDelayUsec = 1;
    ...
    if (mvSFlashInit(sflash, mvSflashInit) )
    {
