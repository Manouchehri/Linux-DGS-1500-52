...
void __init mv_pci_preinit(void)
{
    ...
    if( featureSetP->numOfPex == 0)
	{
    	printk( "PEX not supported\n");
		return;
	}
...
int __init mv_pci_setup(int nr, struct pci_sys_data *sys)
{
    struct resource *res;
    MV_CHIP_FEATURES *featureSetP = mvChipFeaturesGet();
    extern unsigned long pex0_mem_base;
    if( featureSetP->numOfPex == 0)
	{
    	printk( "PEX not supported\n");
		return 0;
        ...
        MV_REG_WRITE(0x20014, pex0_mem_base);
        MV_REG_WRITE(0x20018, pex0_mem_base);
