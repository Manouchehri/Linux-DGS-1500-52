 ...
 *
 * Description:
 *      Enumerates all supported mapping of RX frame to GbE1 RX queue.
 *
 * Note:
...
/****************************/
/* new for I2C general read */
/****************************/

MV_STATUS bspI2cRead
(
    IN MV_U8           devId,       /* I2c slave ID                              */ 
    IN MV_U8           *pData,      /* Pointer to array of chars (address / data)*/
    IN MV_U8           len,         /* pData array size (in chars).              */
    IN MV_BOOL         stop,         /* Indicates if stop bit is needed in the end  */
    IN MV_BOOL	validOffset,
    IN MV_U16	offset,
    IN MV_BOOL	morethan256
);

/****************************/
/* new for I2C general write */
/****************************/

MV_STATUS bspI2cWrite
(
    IN MV_U8           devId,       /* I2c slave ID                              */ 
    IN MV_U8           *pData,      /* Pointer to array of chars (address / data)*/
    IN MV_U8           len,         /* pData array size (in chars).              */
...
*
* DESCRIPTION:
*       Registers RX ISR callback function which is called when GbE1 sets
...
*
* DESCRIPTION:
*       Registers TX_DONE ISR callback function which is called when GbE1 sets
...
*       to the GbE1 RX SDMA descriptors.
*
* INPUTS:
*       pktInfoP  - represents frame (possibly Scatter-Gather).
*       rxQ       - RX queue
...
*
* INPUTS:
*       MV_BOOL enable ï¿?when MV_TRUE MUX will be enabled, MV_FALSE ï¿?disabled.
*
* OUTPUTS:
*       None.
*
...
*
* INPUTS:
*       MV_U32 portsBmp ï¿?ports bit map.
*
* OUTPUTS:
*       None.
*
